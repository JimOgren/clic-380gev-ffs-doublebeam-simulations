...
# Sextupole fine tuning, 1e5 particles
##########################################################

proc sextupole_knobs_1e5 { range machine } {
	puts " "
	puts "Starting tuning with pairs. Fine tuning. GP Parallel exectuion."
   ...
   foreach knob { 1 2 3 } {
		...
		exec bash -c "for i in `seq 1 1 10`  ; do ./run_guinea_parallel.sh 1 1 \$i & done"
		exec bash -c "for i in `seq 11 1 20`  ; do ./run_guinea_parallel.sh 1 1 \$i & done"
		exec bash -c "for i in `seq 21 1 29`  ; do ./run_guinea_parallel.sh 1 1 \$i & done"
...
# Octupole fine tuning, 1e5 particles
##########################################################
proc octupole_knobs_1e5 { range machine } {
	puts " "
	puts "Starting tuning with pairs. Octupole fine tuning. GP Parallel execution."
	exec bash -c "rm -rf temp_*" # remove temp folders
	exec bash -c "mkdir -p GP_output_temp/" # create temp folder
	exec bash -c "mkdir -p beams_temp/" # create temp folder
   foreach knob { 1 2 } {
