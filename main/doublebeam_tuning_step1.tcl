...
puts "Tuning with beamstrahlung - STEP 1 - Beamstrahlung Random Walk"
...
# load machines after BBA
#############################################################################
load_beamline_status "electron" $res_dir/Results_BBA/machine_status_electron_$machine.dat
load_beamline_status "positron" $res_dir/Results_BBA/machine_status_positron_$machine.dat

Octave {
	disp("turn octupoles OFF")
	MSoct_e = placet_element_get_attribute("electron", MIoct, "strength");
	MSoct_p = placet_element_get_attribute("positron", MIoct, "strength");
	placet_element_set_attribute("electron", MIoct, "strength", zeros(size(MIoct)));
	placet_element_set_attribute("positron", MIoct, "strength", zeros(size(MIoct)));
}
...
# Sextupole random walk tuning, beamstrahlung power
#############################################################################
set iter [random_walk_beamstrahlung_power $subset]
puts "iter = $iter"
...
Octave {
	...
	eval(["save ", fname," LL iter "]);

	Lumis = load("LuminosityData.dat")	;
	fname = ["$save_dir", "/LuminosityData_" ,num2str($machine),".dat"]
	eval(["save ", fname," Lumis"]);

	# Turn octupoles ON before saving machine status
	placet_element_set_attribute("electron", MIoct, "strength", MSoct_e);
	placet_element_set_attribute("positron", MIoct, "strength", MSoct_p);
